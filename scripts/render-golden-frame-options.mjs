...
await page.goto(`file://${path.join(printDir, "golden-frame-options.html")}`, { waitUntil: "networkidle" });
await page.evaluate(() => document.fonts?.ready);
await page.pdf({ path: path.join(outputDir, "zach-cowan-golden-frame-options.pdf"), format: "Letter", printBackground: true, preferCSSPageSize: true });
await page.screenshot({ path: path.join(outputDir, "zach-cowan-golden-frame-options.png"), fullPage: true });
await browser.close();
console.log(`Rendered golden frame options to ${outputDir}`);
